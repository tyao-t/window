...
def forwardFlow():
    ...
    with ThreadPoolExecutor(8) as executor:
        ...
        while True:
            ...
            futures.append(executor.submit(processPacket, packet, True))
...
def backwardFlow():
    ...
    with ThreadPoolExecutor(8) as executor:
        ...
        while True:
            ...
            futures.append(executor.submit(processPacket, packet, False))
        for future in futures:
            future.result()
    #executor.shutdown(wait=True, cancel_futures=False)
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(2) as g_executor:
        futures = []
            # submit tasks and collect futures
        futures.append(g_executor.submit(forwardFlow))
        futures.append(g_executor.submit(backwardFlow))
        # wait for all tasks to complete
        for future in futures:
            future.result()
    #g_executor.shutdown(wait=True, cancel_futures=False)
